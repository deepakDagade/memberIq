...
public class MembershipSpecification implements Specification<Membership> {

	private static final long serialVersionUID = 1L;

	private final FilterCriteria criteria;

	public MembershipSpecification(FilterCriteria criteria) {
		this.criteria = criteria;
	}

	@Override
	public Predicate toPredicate(Root<Membership> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
		switch (criteria.getKey()) {
		case GlobalConstants.DISCOUNT:
			return criteriaBuilder.equal(root.get(GlobalConstants.DISCOUNT), criteria.getValue());

		case GlobalConstants.TOTAL_AMOUNT:
			return criteriaBuilder.equal(root.get(GlobalConstants.TOTAL_AMOUNT), criteria.getValue());
